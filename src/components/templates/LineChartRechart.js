...
function LineChartRechart({ title = "차트 제목", type }) {
   ...
   // console.log(type, smartPipeList);
      ...
      <>
         <LineChartStyle.Wrap>
            ...
            <div style={{ padding: "0 20px 40px 40px" }}>
               ...
               {/* 범례 */}
               <LegendList dcacData={smartPipeList} list={list} onChange={onChange} />

               {/* 라인 차트 */}
               <LineChartStyle.ChartWrap>
                  <ResponsiveContainer width={"100%"} height={284}>
                     <LineChart data={smartPipeList}>
                        ...
                        {!isEmpty(smartPipeList)
                           ? smartPipeList?.map((detData, i) => (
                                <Line
                                   //   r={0}
                                   key={i}
                                   data={type === "pp" ? detData?.ppList : type === "optical" ? detData?.opticalList : type === "waves" ? detData?.wavesList : []}
